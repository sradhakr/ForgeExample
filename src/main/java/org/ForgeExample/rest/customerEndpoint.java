...
import org.ForgeExample.model.customer;
...
public class customerEndpoint {
	@PersistenceContext(unitName = "ForgeExample-persistence-unit")
	...
	public Response findById(@PathParam("id") Long id) {
		TypedQuery<customer> findByIdQuery = em
				.createQuery(
						"SELECT DISTINCT c FROM customer c LEFT JOIN FETCH c.orderid WHERE c.id = :entityId ORDER BY c.id",
						customer.class);
	...
	public List<customer> listAll(@QueryParam("start") Integer startPosition,
		...
		TypedQuery<customer> findAllQuery = em
				.createQuery(
						"SELECT DISTINCT c FROM customer c LEFT JOIN FETCH c.orderid ORDER BY c.id",
						customer.class);
